...
public class AnyFiveActiviesCrawler20 {
    ...
    private static final String INSERT_SQL =
            "INSERT INTO approval_data_2020 (document_id, post_title, sequence, status, approval_date, department, approver) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?)";
...
// 문서 ID를 가져올 SQL 쿼리 - 이미 처리된 문서 제외
    private static final String SELECT_DOCUMENT_IDS_SQL =
            "SELECT source_id FROM document_sources " +
                    "WHERE end_year = 2020 " +
                    "AND source_id NOT IN (SELECT DISTINCT document_id FROM approval_data_2020) " +
                    "ORDER BY source_id";
